...
const FormEditTask: React.FC<{ formId: string; existing: Task | undefined; onSuccess: () => void }> = ({ formId, existing, onSuccess }) => {
  ...
  async function submit(data: z.infer<typeof schema>) {
    try {
      await axios.put(`/api/tasks/${existing?.id}`, data);
      ...
      if (error instanceof AxiosError) {
        ...
        toast.success(`Failed to update task. ${error.response?.data.message}`);
      }
    }
  }
  return (
